...
export const iTextMessage = async (
  ...
  console.log(payload)
  const { type, msgType, msgParams, refId, timestamp, sentBy, tempId } = payload

  const { participants } = await db.collection(type).findOne({
    _id: new ObjectId(refId),
  })

  await db.collection(COLLECTION.MESSAGE).insertOne(
    {
      type,
      msgType,
      msgParams,
