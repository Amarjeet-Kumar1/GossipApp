...
export const AvatarSection = ({ otherFriend }: any) => {
  const {
    dropDown,
    authUsers,
    chat,
  ...
  const [newName, setNewName] = useState(
    chat[activeChat].chatInfo.name ?? authUsers[otherFriend]?.displayName ?? ""
  )
  const [hoverForNewAvatar, setHoverForNewAvatar] = useState(false)

  const updateGroupInfo = (payload: any) => {
  ...
  const handleDropMenuClicks = (e: any, type: string) => {
    setDropDown({
      ...
      params: {
        ...
        src: type === "changeAvatar" ? chat[activeChat].chatInfo?.avatar : null,
      },
    })
  }
  ...
  return (
    <div className={s.avatarSection}>
      <div className={s.infoModalAvatar}>
        ...
        (otherFriend && authUsers[otherFriend]?.avatar) ? (
          <div
